# راهنمای یکپارچه‌سازی SIP و NAT
...
- [راهنمای یکپارچه‌سازی SIP و NAT](#راهنمای-یکپارچهسازی-sip-و-nat)
  ...
  - [چرا NAT با SIP مشکل ایجاد می‌کند؟](#چرا-nat-با-sip-مشکل-ایجاد-میکند)
    - [نحوه کار SIP و RTP](#نحوه-کار-sip-و-rtp)
    - [تفاوت با پروتکل‌های سازگار با NAT](#تفاوت-با-پروتکلهای-سازگار-با-nat)
  ...
  - [دو روش اصلی برای عبور از سد NAT](#دو-روش-اصلی-برای-عبور-از-سد-nat)
    - [1. روش سمت کاربر](#1-روش-سمت-کاربر)
    - [2. روش سمت سرور](#2-روش-سمت-سرور)
    - [مثال ساده](#مثال-ساده)
    - [به طور خلاصه](#به-طور-خلاصه)
    - [کدام روش بهتر است؟](#کدام-روش-بهتر-است)
  - [رویکرد سمت سرور برای حل مشکل NAT (موضوع Server-side NAT traversal)](#رویکرد-سمت-سرور-برای-حل-مشکل-nat-موضوع-server-side-nat-traversal)
  ...
  - [مشکلات ALG (Application Layer Gateways) و چرا آن‌ها را دوست نداریم!](#مشکلات-alg-application-layer-gateways-و-چرا-آنها-را-دوست-نداریم)
    ...
    - [چگونه با ALGها مقابله کنیم؟](#چگونه-با-algها-مقابله-کنیم)
      - [۱. غیرفعال کردن ALG](#۱-غیرفعال-کردن-alg)
      - [۲. تعویض روتر](#۲-تعویض-روتر)
      - [۳. استفاده از پورت‌های غیراستاندارد](#۳-استفاده-از-پورتهای-غیراستاندارد)
    - [جمع‌بندی](#جمعبندی-2)
  ...
  - [چه زمانی به RTP Relay نیاز داریم؟](#چه-زمانی-به-rtp-relay-نیاز-داریم)
    - [۱. وقتی RTP Relay لازم نیست](#۱-وقتی-rtp-relay-لازم-نیست)
    - [۲. وقتی RTP Relay لازم است](#۲-وقتی-rtp-relay-لازم-است)
    - [جمع‌بندی](#جمعبندی-4)
  ...
  - [مثال عملی: تنظیمات شبکه و دستگاه‌ها](#مثال-عملی-تنظیمات-شبکه-و-دستگاهها)
    ...
    - [چرا این تنظیمات پیچیده است؟](#چرا-این-تنظیمات-پیچیده-است)
    - [جمع‌بندی](#جمعبندی-6)
  - [ماژول nathelper در Kamailio](#ماژول-nathelper-در-kamailio)
    - [۱. تغییر ویژگی‌های پیام‌های SIP برای سازگاری با NAT](#۱-تغییر-ویژگیهای-پیامهای-sip-برای-سازگاری-با-nat)
    - [۲. تشخیص دستگاه‌های پشت NAT](#۲-تشخیص-دستگاههای-پشت-nat)
    - [۳. ارسال پیام‌های Keepalive به دستگاه‌های پشت NAT](#۳-ارسال-پیامهای-keepalive-به-دستگاههای-پشت-nat)
    - [ارتباط بین nathelper و ماژول ثبت‌کننده (registrar)](#ارتباط-بین-nathelper-و-ماژول-ثبتکننده-registrar)
    - [تابع nat\_uac\_test()](#تابع-nat_uac_test)
    - [جمع‌بندی](#جمعبندی-7)
  - [مثال عملی: یک درخواست REGISTER از دستگاه پشت NAT](#مثال-عملی-یک-درخواست-register-از-دستگاه-پشت-nat)
    - [جزئیات درخواست](#جزئیات-درخواست)
    - [تشخیص دستگاه‌های پشت NAT با nat\_uac\_test()](#تشخیص-دستگاههای-پشت-nat-با-nat_uac_test)
  ...
  - [تفاوت بین روش‌های قدیمی و جدید برای حل مشکل NAT در Kamailio](#تفاوت-بین-روشهای-قدیمی-و-جدید-برای-حل-مشکل-nat-در-kamailio)
    - [تابع fix\_nated\_contact()](#تابع-fix_nated_contact)
    - [تابع fix\_nated\_register()](#تابع-fix_nated_register)
    - [روش‌های جدید: handle\_ruri\_alias() و set\_contact\_alias()](#روشهای-جدید-handle_ruri_alias-و-set_contact_alias)
      - [چگونه کار می‌کنند؟](#چگونه-کار-میکنند)
    - [جمع‌بندی](#جمعبندی-9)
  - [درک عمیق‌تر کد Kamailio برای مدیریت NAT در تماس‌های SIP](#درک-عمیقتر-کد-kamailio-برای-مدیریت-nat-در-تماسهای-sip)
    ...
    - [بخش سوم: مدیریت درخواست‌های ثبت‌نام (REGISTER)](#بخش-سوم-مدیریت-درخواستهای-ثبتنام-register)
    - [اهمیت هر بخش از کد:](#اهمیت-هر-بخش-از-کد)
    - [چگونه این کد کار می‌کند؟](#چگونه-این-کد-کار-میکند)
    - [کاربردهای این کد:](#کاربردهای-این-کد)
  - [مدیریت سرورهای پشت NAT در محیط‌های ابری](#مدیریت-سرورهای-پشت-nat-در-محیطهای-ابری)
    ...
    - [توجه:](#توجه)
    - [به زبان ساده‌تر:](#به-زبان-سادهتر)
    - [به طور خلاصه:](#به-طور-خلاصه-1)
  - [انتقال صدا بین دو شبکه متفاوت با RTPEngine](#انتقال-صدا-بین-دو-شبکه-متفاوت-با-rtpengine)
    ...
    - [مثال:](#مثال-1)
    - [به زبان ساده‌تر:](#به-زبان-سادهتر-1)
    - [چرا این مهم است؟](#چرا-این-مهم-است)
...
### تفاوت با پروتکل‌های سازگار با NAT
...
* متد **`set_contact_alias()`:** این تابع برای برخی از سناریوهای خاص استفاده می‌شود و ممکن است برای همه پیاده‌سازی‌ها ضروری نباشد. این تابع معمولاً برای مدیریت آدرس‌های Alias در هدر Contact استفاده می‌شود.
...
### بخش سوم: مدیریت درخواست‌های ثبت‌نام (REGISTER)
...
این بخش برای مدیریت درخواست‌های ثبت‌نام استفاده می‌شود. در اینجا، Kamailio بررسی می‌کند که آیا دستگاهی که درخواست ثبت‌نام را ارسال کرده است، قبلاً ثبت‌نام شده است یا خیر. اگر دستگاه قبلاً ثبت‌نام شده باشد، Kamailio اطلاعات مربوط به دستگاه را به‌روزرسانی می‌کند.

### اهمیت هر بخش از کد:

* **تشخیص NAT:** این بخش اولین قدم برای مدیریت تماس‌های SIP در محیط‌های NAT است.
* **مدیریت SDP:** این بخش برای اطمینان از اینکه اطلاعات رسانه‌ای به درستی بین دستگاه‌ها رد و بدل می‌شود، بسیار مهم است.
* **مدیریت ثبت‌نام:** این بخش برای مدیریت اطلاعات مربوط به دستگاه‌های ثبت‌نام شده استفاده می‌شود.

### چگونه این کد کار می‌کند؟

1. زمانی که یک درخواست SIP به سرور Kamailio ارسال می‌شود، این کد اجرا می‌شود.
2. کد ابتدا بررسی می‌کند که آیا درخواست از یک دستگاه پشت NAT آمده است یا خیر.
3. اگر درخواست از پشت NAT آمده باشد، کد تغییرات لازم را در درخواست ایجاد می‌کند تا اطمینان حاصل شود که تماس به درستی برقرار شود.
4. سپس، درخواست به بخش‌های بعدی پردازش ارسال می‌شود.

### کاربردهای این کد:

* **VoIP:** برای مدیریت تماس‌های VoIP در محیط‌هایی که NAT وجود دارد.
* **سرویس‌های ارتباطی:** برای ارائه خدمات ارتباطی مانند پیام‌رسانی، کنفرانس و ویدئو کنفرانس.
...
* ماژول **RTPEngine:** ابزاری برای مدیریت جریان‌های صوتی (RTP)
* **شبکه داخلی:** شبکه‌ای که داخل یک سازمان یا ساختمان وجود دارد.
* **شبکه عمومی:** اینترنت
* **رابط:** مسیری که برای ارتباط بین دو شبکه استفاده می‌شود.
* **جهت انتقال:** مشخص می‌کند که صدا از کدام شبکه وارد و از کدام شبکه خارج می‌شود.
...
**راه حل اول: کاهش زمان ثبت‌نام مجدد**

*   **ایده:** دستگاه‌های تلفنی به طور مرتب به سرور SIP اعلام کنند که آنلاین هستند (ثبت‌نام کنند).
*   **مشکلات:**
    *   **فشار روی سرور:** تعداد زیادی درخواست ثبت‌نام به سرور وارد می‌شود.
    *   **محدودیت‌های دستگاه‌ها:** برخی دستگاه‌ها از ثبت‌نام‌های خیلی مکرر پشتیبانی نمی‌کنند.
...
*   **ثبت‌نام (Registration):** فرآیندی که طی آن یک دستگاه تلفنی به سرور SIP معرفی می‌شود.
*   **Keepalive:** پیامی برای حفظ یک اتصال شبکه
*   **CRLF:** Carriage Return Line Feed
*   **SIP OPTIONS:** یک درخواست SIP برای بررسی قابلیت‌های دستگاه
...
* **مدیریت بهتر NAT:** TCP با مکانیزم‌های مدیریت اتصال و تایید دریافت، مشکلات مربوط به NAT را بهتر حل می‌کند.
...
پس SIP Outbound یک گام مهم در جهت بهبود کیفیت و پایداری ارتباطات SIP است. با این حال، برای اینکه این استاندارد به طور گسترده مورد استفاده قرار گیرد، نیاز به همکاری و هماهنگی بین تولیدکنندگان دستگاه‌ها و توسعه‌دهندگان نرم‌افزارهای SIP است.
