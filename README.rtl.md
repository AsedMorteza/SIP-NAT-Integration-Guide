...
# چرا NAT (ترجمه آدرس شبکه) با SIP (پروتکل آغاز جلسه) مشکل ایجاد می‌کند؟
...
برای مدیریت انتقال صدا (RTP)، ابزاری به نام **RTPEngine** (توسط شرکت Sipwise توسعه داده شده) بسیار محبوب است. این ابزار یک رله RTP بسیار قدرتمند است که می‌تواند صدا را با سرعت نزدیک به سرعت واقعی شبکه (wire speed) منتقل کند. دلیل این سرعت بالا این است که RTPEngine کار انتقال داده‌ها را در فضای هسته (kernel space) سیستم عامل انجام می‌دهد.
...
* **پایداری ارتباط:** حفظ نگاشت‌های NAT و جلوگیری از قطع ارتباط.
